...
def main():
    ...
    parser.add_argument("--tts-engine", type=str, choices=["edge", "kokoro", "chattts", "omnivoice", "mlx"], default="edge", help="TTS engine to use (default: edge)")
    ...
    if args.tts:
        ...
        try:
            ...
            elif args.tts_engine == "mlx":
                from tts.mlx_processor import run_mlx_tts_sync
                voice = args.voice or ""
                audio_path, json_path = run_mlx_tts_sync(args.tts, str(output_base), voice=voice)
            else:
